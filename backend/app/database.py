...
print(f"[Database] Connecting to: {db_url}", flush=True)
...
# Double-insure by passing auth_token directly to connect_args (stripped of potential quotes)
if "turso.io" in db_url:
    ...
    if token:
        connect_args["auth_token"] = token.strip("\"'")
